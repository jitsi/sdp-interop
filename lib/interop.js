...
Interop.prototype.toPlanB = function(desc) {
    ...
    media.forEach(function(unifiedLine) {
        ...
        // If we don't have a channel for this unifiedLine.type, then use this
        // unifiedLine as the channel basis.
        ...
        if (typeof unifiedLine.sources === 'object') {
            Object.keys(unifiedLine.sources).forEach(function(ssrc) {
                ...
                type2bl[unifiedLine.type].sources[ssrc] =
                    unifiedLine.sources[ssrc];

                if (typeof unifiedLine.msid !== 'undefined') {
                    // In Plan B the msid is an SSRC attribute. Also, we don't
                    // care about the obsolete label and mslabel attributes.
                    //
                    // Note that it is not guaranteed that the unifiedLine will
                    // have an msid. recvonly channels in particular don't have
                    // one.
                    type2bl[unifiedLine.type].sources[ssrc].msid =
                        unifiedLine.msid;
                }
        ...
        if (typeof unifiedLine.ssrcGroups !== 'undefined' &&
            ...
            type2bl[unifiedLine.type].ssrcGroups =
                type2bl[unifiedLine.type].ssrcGroups.concat(
                    unifiedLine.ssrcGroups);
        }
...
Interop.prototype.toUnifiedPlan = function(desc) {
    ...
    if (desc.type === 'answer') {
        ...
        // to be careful with this approach because inactive m-lines do not
        ...
        for (var i = 0; i < cached.media.length; i++) {
            ...
            if (typeof mid2ul[unifiedLine.mid] === 'undefined') {
                ...
                else if (unifiedLine.direction === 'sendonly')
                    unifiedLine.direction = 'inactive';
            } else {
    ...
    ["audio", "video"].forEach(function (type) {
        ...
        if (Object.keys(recvonlySsrcs[type]).length > 0) {
            ...
            if (idx === null){
                // If this is the first offer we receive, we don't have a cached
                // answer. Assume that we will be sending media using the first
                // m-line for each media type.
                for (var i = 0; i < session.media.length; i++) {
                    if (session.media[i].type === type) {
                        idx = i;
                        break;
